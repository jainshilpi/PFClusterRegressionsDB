...

{
  ...
  payloadfile = iConfig.getParameter<std::string>("payloadfile");
  payloadname = iConfig.getParameter<std::string>("payloadname");
  payloadobj = iConfig.getParameter<std::string>("payloadobj");
...
GBRWrapperMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  TFile *infile = new TFile(payloadfile.c_str(),"READ");
  printf("load forest\n");
  
  //GBRForestD *p4comb = (GBRForest*)infile->Get("CombinationWeight");
  GBRForestD *gbreb = (GBRForestD*)infile->Get(payloadobj.c_str());
  //  GBRForestD *gbrebvar = (GBRForestD*)infile->Get("EBCorrection");
  //  GBRForestD *gbree = (GBRForestD*)infile->Get("EBUncertainty");
  //  GBRForestD *gbreevar = (GBRForestD*)infile->Get("EBUncertainty");
